...
{ 
    ...
    public override bool DiscardItem(Vector3 discardPosition)
    {
        ...
        grapplingHookLeft.gameObject.SetActive(false);
        grapplingHookRight.gameObject.SetActive(false);
        grapplingHookGearLeftModelTransform.gameObject.SetActive(true);
        grapplingHookGearRightModelTransform.gameObject.SetActive(true);

        transform.rotation = controller.playerTransform.rotation;
        return base.DiscardItem(discardPosition);
    }
