...
public class GrapplingHookCheckPointController : MonoBehaviour
{
    ...
    private void Update()
    {
        if (!(EquipmentsController.Instance is null) && 
            EquipmentsController.Instance.nowMovementEquipmentIndex != -1 &&
            EquipmentsController.Instance.nowMovementEquipment.equipmentName == E_EquipmentName.GrapplingHookGears)
        {
            _grapplingHookGears = EquipmentsController.Instance.nowMovementEquipment as GrapplingHookGears;
